...
-- jujuy_utn--
    ...
    f.sexo AS gender,
    date_part('year',age(TO_DATE(f.birth_date,'YYYY-MM-DD'))) AS age,
    l.codigo_postal AS postal_code,
    f.location AS location,
    f.email AS email	
FROM jujuy_utn f
LEFT JOIN localidad2 l
ON upper(l.localidad)= l.localidad
WHERE university LIKE 'universidad_tecnologica_nacional'
AND TO_DATE(f.inscription_date,'YYYY-MM-DD')
